...
class EpubIndexer(object):
    ...
    def load(self, epub, bid):
        ...
        for spineItem in epub.spine:
            ...
            self.engine.add(bid=bid, path=path, href=spineItem['href'], title=spineItem['title'], cfiBase=spineItem['cfiBase'], spinePos=spineItem['spinePos'])

        self.engine.finished()

    def search(self, q, bid, limit=None):
        rawresults = self.engine.query(q, bid, limit)
        r = {}
        r["results"] = []
        q = q.lower()
